        ...
        var settings = {
            ...
            methods: {
                init: function () {
                    ...
                    //绑定事件
                    obj.object.Text.bind("change", function () {
                ...
                showMenu: function () {
                    var obj = settings;
                    var dvMainObj = obj.object.dvMain;
                    var dvMainOffset = obj.object.dvMain.offset();
                    var arrSelValues = obj.object.Value.val().split(obj.defaults.textChar); //需要绑定选中的项的值数组
                    ...
                    var TreeHeight = obj.defaults.Height;

                    //当树形下拉框选项过多时，如果页面下部空白区放不下，则会自动向上展开；

                    //如果上部也放不下，下拉列表的高度会正好到页面可视区域的底部，并出现滚动条；

                    if (TreeVisualDownHeight >= obj.defaults.Height) {
                        TreeTop = dvMainOffset.top + dvMainObj.outerHeight();
                        //alert("aaa" + "/" + TreeVisualDownHeight + "/" + obj.defaults.Height);

                    }
                    else if (TreeVisualUpHeight >= obj.defaults.Height) {
                        TreeTop = dvMainOffset.top - dvMainObj.outerHeight() - obj.defaults.Height + 18;
                        //alert("bbb"+"/" + TreeVisualUpHeight + "/" + obj.defaults.Height);
                    ...
                    obj.object.Content.css({ left: TreeLeft + "px", top: TreeTop + "px" }).slideDown("fast");
                    obj.object.Tree.attr("style", "height:" + TreeHeight + "px;");

                    //设置下拉框宽度                    
                    var dvMainActureWidth = obj.object.dvMain.outerWidth(); //dvMain的宽度

                    var TreeActureWidth = obj.object.Tree.outerWidth(); //Tree的宽度

                    var TreeScrollWidth = obj.object.Tree[0].scrollWidth + 2; //Tree的有滚动条时的宽度

                    //alert(TreeActureWidth + "/" + TreeScrollWidth);
                    //alert(TreeActureWidth + "/" + dvMainActureWidth + "/" + obj.defaults.maxWidth + "/" + obj.defaults.ControlId);
                    if (TreeActureWidth < (dvMainActureWidth - 12)) {
                        //当Tree的宽度少于dvMain的宽度时，调整Tree的宽度等于dvMain的宽度

                        obj.object.Tree.css("width", (dvMainActureWidth - 12) + "px");
                    }
                    else if (TreeActureWidth > obj.defaults.maxWidth) {
                        //当Tree的宽度大于控件的最大宽度时，调整Tree的宽度等于控件的最大宽度

                        obj.object.Tree.css("width", (obj.defaults.maxWidth) + "px");
                    }
                        ...
                        obj.object.Tree.css("width", (TreeActureWidth + 2) + "px");
                    }
                    ...
                    $("html").bind("mousedown", function (e) { obj.methods.onBodyDown(e); });
                },
                hideMenu: function () {
                    var obj = settings;
                    obj.object.Content.fadeOut("fast");
                    $("html").unbind("mousedown", function (e) { obj.methods.onBodyDown(e); });
                },
                onBodyDown: function (event) {
                    //alert("aaa");
                    var obj = settings;
                    //var event = $(event)[0];
                    if (!(event.target.id == (obj.defaults.ControlId + "_Select") || event.target.id == (obj.defaults.ControlId + "_dvMain") || event.target.id == obj.defaults.ControlId + "_Content" || $(event.target).parents("#" + obj.defaults.ControlId + "_Content").length > 0)) {
                        obj.methods.hideMenu();
                    }
                },
                //树形控件数据绑定
